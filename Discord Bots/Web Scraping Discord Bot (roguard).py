...
### This is a Discord bot that searches, scrapes, and sends
### details about monsters, items, etc from https://www.roguard.net/
### on user command

### Sample command from Discord server users:
### "kafra find poring"
### Bot responds with:
### https://www.roguard.net/db/monsters/10001/
### Level: 1
### Race: Plant
### Element: Water
### Size: Medium
### Locations:
### Prontera South Gate (Lv 1 ~ 10)
### Labyrinth Forest (Lv 20 ~ 24)
### Prontera North (Lv -)
...
async def find(ctx, *args):
    ...
    print("rog search", search_item.lower())
    ...
    if results_list is None:
        ...
        if not card_effects_str is None and not dropped_by_str is None:
            await ctx.send("```"+card_effects_str.replace("```", "")[:-1]+dropped_by_str.replace("```", "")+"\n```")
        ...
        ### Equipment dropped by monsters
            ...
            await ctx.send("```"+common_table_str.replace("```", "")[:-1]+dropped_by_str.replace("```", "")+"\n```")
            print("```"+common_table_str.replace("```", "")+dropped_by_str.replace("```", "")+"\n```")
        ### Monster
        elif not common_table_str is None and not locations_str is None:
            await ctx.send("```"+common_table_str.replace("```", "")[:-1]+locations_str.replace("```", "")+"\n```")
            print("```"+common_table_str.replace("```", "")+locations_str.replace("```", "")+"\n```")
        ### Equipment not dropped by monsters
        elif not common_table_str is None:
            await ctx.send(common_table_str)
            print(common_table_str)
        ### Material
        elif not dropped_by_str is None:
            await ctx.send(dropped_by_str)
            print(dropped_by_str)

TOKEN = "hidden for security purposes"
client.run(TOKEN)